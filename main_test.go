...
func TestService(t *testing.T) {
	...
	resp, body, err = executeRequest(http.MethodPost, "http://localhost:8080/new", bytes.NewReader([]byte(`{"id":"test","port":8090, "responses":[{"url": "/url", "code":200, "response": "ok"},{"url": "/url", "code":200, "response": "ok"}]}`)))
	require.NoError(t, err)
	require.Equal(t, http.StatusBadRequest, resp.StatusCode)
	...
	require.Equal(t, `{"id": "test"}`, string(body))
	...
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	require.Equal(t, `{"ID":"test","host":"","port":8090,"forward-url":"","passthrough-re":"^$","url-re":"^.*$","body-re":"^.*$","responses":[{"id":"766C4650CB3E23432941DDCD3D663883BA3AF05C5DA44D79074657EB776B99C3","url":"/url","code":200,"response":"ok"}]}`, string(body))
	cfg := body
	...
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	require.Equal(t, `{"id":"766C4650CB3E23432941DDCD3D663883BA3AF05C5DA44D79074657EB776B99C3","url":"/url","code":200,"response":"ok"}`, string(body))
	response := body
