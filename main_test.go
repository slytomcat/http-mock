...
func TestService(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, http.StatusBadRequest, resp.StatusCode)
	resp, _, err = executeRequest(http.MethodGet, fmt.Sprintf("http://localhost:8080/response?id=%s&resp-id=888BAD1BAD1E23432941DDCD3D663883BA3AF05C5DA44D79074657EB776B99C3", data.ID), nil)
	require.NoError(t, err)
	require.Equal(t, http.StatusBadRequest, resp.StatusCode)
