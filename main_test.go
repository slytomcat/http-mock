...
func TestService(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	resp, _, err = executeRequest(http.MethodGet, "http://localhost:8080/dump_configs", nil)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	resp, _, err = executeRequest(http.MethodGet, "http://localhost:8080/load_configs", nil)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	data.ID = "wrong"
	resp, body, err = executeRequest(http.MethodGet, fmt.Sprintf("http://localhost:8080/start?id=%s", data.ID), nil)
